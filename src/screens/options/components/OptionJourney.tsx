...
// Shows options that started at ~₹1k — ATL, ATH and travel % from ATL.
...
export function OptionJourney() {
  ...
  async function loadBatchIds() {
    try {
      const res = await optionJourneyService.getBatchIds();
  ...
  async function loadBatch(batchId = selectedBatch, silent = false) {
    ...
    try {
      ...
      setData((res.data ?? []).map((o: any) => ({
        ...
        volume: o.volume,
        atl_ath:
          o.all_time_low != null && o.all_time_high != null
  ...
  async function fetchFresh() {
    setLoading(true);
    try {
      const res = await optionJourneyService.fetchFresh();
    ...
    Alert.alert(
      ...
      [
        { text: 'Cancel', style: 'cancel' },
        {
          ...
          onPress: async () => {
            try {
              const res = await optionJourneyService.deleteBatch(selectedBatch);
